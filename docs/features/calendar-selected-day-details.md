...
- `CalendarAgendaView` selects intrinsic content or a 256-point scrolling viewport in the same layout pass, avoiding an estimated list height followed by asynchronous measurement. `CalendarComponentView` measures the complete intrinsic content before the host frame; `CalendarPlugin.descriptor` rounds up to the host's 8-point grid and notifies the host. The fixed six-row month content is 298 points (304 points after grid rounding). Library previews do not resize live widgets.
